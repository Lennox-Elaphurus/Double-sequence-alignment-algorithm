...
sequence2 ="AAA"# "VGAWAD"
# sequence2="0"+sequence2
print("sequence2:", sequence2)
# finish reading two sequence
# build score matrix
GAP = -8
score = [[0 for i in range(len(sequence1)+1)] for i in range(len(sequence2)+1)]
# use a 4-D list to represent whether two node are connected
connectivity= [[[[False for i in range(len(sequence1)+1)] for i in range(len(sequence2)+1)] for i in range(len(sequence1)+1)] for i in range(len(sequence2)+1)]
...
def valueFromBLOSUM(i1, i2):
    ...
    global sequence1
    global sequence2
    charX= sequence1[i2-1]   # the key to find the value
    charY= sequence2[i1-1]   # the key to find the value
...
for i1 in range(1, len(sequence2)):
    for i2 in range(1, len(sequence1)):
        score[i1][i2]= max(score[i1-1][i2]+GAP, score[i1][i2-1]+GAP, score[i1-1][i2-1]+valueFromBLOSUM(i1, i2))
        ...
        if score[i1][i2] == score[i1-1][i2-1]+valueFromBLOSUM(i1, i2):
        ...
        for i4 in range(len(sequence1)):
            if connectivity[i1][i2][i3][i4] == True and i3 <= i1 and i4 <= i2:
                if i3 == i1-1 and i4 == i2-1:  # come from diag
                    pathX.insert(0, sequence1[i2-1])
                    pathY.insert(0, sequence2[i1-1])
